...
REAL_XT_BB_CONFIG_CMD = "${@d.getVar('XT_BB_CONFIG_CMD') or '. poky/scripts/oe-setup-builddir'}"
REAL_XT_BB_RUN_CMD = "${@d.getVar('XT_BB_RUN_CMD') or '. poky/scripts/oe-setup-builddir'}"

export BUILDDIR="${S}/build"

build_yocto_configure() {
    local local_conf="${S}/build/conf/local.conf"
    if [ -d "${S}/poky"  ] ; then
      export OEROOT="${S}/poky"
    else
      export OEROOT="${S}/sources/poky"
    fi
    cd ${S}
